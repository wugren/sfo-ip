...
use crate::error::{Ip2RegionError, MakerError, Result};
...
impl TryFrom<&[u8; 256]> for Header {
    ...
    fn try_from(value: &[u8; 256]) -> Result<Self> {
        if value.len() < 20 {
            return Err(Ip2RegionError::MakerError(MakerError::HeaderParsed("Header bytes too short".into())));
        }
        ...
        Ok(Header {
            ...
            ip_version: IpVersion::from_u16(ip_version_value).ok_or_else(|| {
                Ip2RegionError::MakerError(MakerError::HeaderParsed(format!("Header ip version invalid: {ip_version_value}")))
            })?,
